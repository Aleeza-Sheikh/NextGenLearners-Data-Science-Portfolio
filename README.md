...
| Week | Project | Focus |
|---|---|---|
| [Week 1](./week1-data-cleaning) | Applicants Data Cleaning | Cleaning a messy applicant dataset — missing values, duplicates, inconsistent formatting — into an analysis-ready CSV |
...
**Key work:** dropped rows missing critical identifiers, removed 3 duplicate rows, standardized `Domain Applied`/`Status`/`University` text casing, mapped inconsistent domain spellings into 5 clean categories, converted `Application Date` to a proper datetime, and cleaned phone number formatting.
